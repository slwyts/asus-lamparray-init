...
这类键盘在 Windows 初始化后热重启到 Linux 通常可以正常亮起，但长时间关机再直接启动 Linux 时可能保持熄灭。原因是当前 `asusctl` 没有发现位于 I2C-HID 总线上的 LampArray 控制器。本工具直接使用标准 HID LampArray Feature Report 完成初始化；之后 KDE 仍可通过系统键盘亮度控制器调节亮度。
...
- 安装命令到 `/usr/local/bin/asus-lamparray-init`
- 创建 `/etc/asus-lamparray-init.conf`
- 安装并启用 `asus-lamparray-init.service`
- 立即尝试初始化键盘灯
...
默认只匹配 ASUS `0B05:19B6` I2C-HID LampArray，不会向其他 HID 设备写入数据。
